...
export default function BasicTab({ data, loading, accent, onAdd, onUpdate, onDelete, uid, theme }) {
  ...
  const uploadFile = async (item, file) => {
    ...
    try {
      const fileRef = ref(storage, `users/${uid}/basic/${item.id}/${file.name}`);
  ...
  const handleFileDelete = async (item) => {
    ...
    try {
      const fileRef = ref(storage, `users/${uid}/basic/${item.id}/${item.fileName}`);
